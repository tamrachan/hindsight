export const CATEGORY_COLORS = {
  "market crash": "#ef4444",
  "geopolitical shock": "#f97316",
  "banking crisis": "#eab308",
  pandemic: "#3b82f6",
  "monetary policy": "#a855f7",
  sustainability: "#22c55e",
};

export const CATEGORY_BADGE_CLASSES = {
  "market crash": "bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300",
  "geopolitical shock": "bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300",
  "banking crisis": "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300",
  pandemic: "bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300",
  "monetary policy": "bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300",
  sustainability: "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300",
};
